...
class Intcoder:
    ...
    def parse_instruction(self):
        ...
        instruction = "0"*(5 - len(str(self.data[self.pc]))) + str(self.data[self.pc])
        ...
        for i in range(self.num_params[self.opcode]):
            if int(instruction[parameter_base-i]) == 1:
                params[i] = self.pc+i+1
            elif int(instruction[parameter_base-i]) == 2:
                params[i] = self.data[self.pc+i+1] + self.relative_base
            else:
                params[i] = self.data.get(self.pc+i+1, 0 )
    ...
    def op_jump_if_true(self, p1, p2, *args):
        self.pc = self.data.get(p2, 0) if self.data.get(p1, 0) != 0 else self.pc+3
